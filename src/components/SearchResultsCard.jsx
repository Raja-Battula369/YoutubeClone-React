...
const SearchResultsCard = ({video}) => {
  ...
  return (
    <Link to={`/video/${video?.videoId}`}>
        <HStack  fontSize={["xx-small","small","md"]} alignItems="flex-start"  maxW={"80vw"} gap="0.1rem"   color={colorMode==="light"?"black":"white"}>
                
            <div>
                <Image src={video.thumbnails[0].url} borderRadius={"xl"}
                maxW={["8rem","20rem"]}  alt='Video' objectFit={"contain"} />
                
                {video?.lengthSeconds &&
                (<Badge fontSize={["xx-small","small","md"]} color={colorMode==="light"?"black":"white"}  pos={"relative"} top={["-1rem","-2rem"]} left={["5rem","14rem"]}>
                ...
                <VStack  maxW={"50rem"} alignItems={"flex-start"}>
                    ...
                    <Text color={colorMode==="light"?"black":"white"} noOfLines={1} >{video?.descriptionSnippet} </Text>
